a = {
    "Центральный":       { "Белгородская область":                        0,
                           "Брянская область":                            0,
                           "Владимирская область":                        0,
                           "Воронежская область":                         0,
                           "Ивановская область":                          0,
                           "Калужская область":                           0,
                           "Костромская область":                         0,
                           "Курская область":                             0,
                           "Липецкая область":                            0,
                           "Московская область":                          0,
                           "Орловская область":                           0,
                           "Рязанская область":                           0,
                           "Смоленская область":                          0,
                           "Тамбовская область":                          0,
                           "Тверская область":                            0,
                           "Тульская область":                            0,
                           "Ярославская область":                         0,
                           "Город федерального значения Москва":          0 },
    "Северо-Западный":   { "Республика Карелия":                          0,
                           "Республика Коми":                             0,
                           "Архангельская область":                       0,
                           "Вологодская область":                         0,
                           "Калининградская область":                     0,
                           "Ленинградская область":                       0,
                           "Мурманская область":                          0,
                           "Новгородская область":                        0,
                           "Псковская область":                           0,
                           "Город федерального значения Санкт-Петербург": 0,
                           "Ненецкий автономный округ":                   0 },
    "Южный":             { "Республика Адыгея":                           0,
                           "Республика Калмыкия":                         0,
                           "Республика Крым":                             0,
                           "Краснодарский край":                          0,
                           "Астраханская область":                        0,
                           "Волгоградская область":                       0,
                           "Ростовская область":                          0,
                           "Луганская Народная Республика":               0,
                           "Донецкая Народная Республика":                0,
                           "Запорожская область":                         0,
                           "Херсонская область":                          0,
                           "Город федерального значения Севастополь":     0 },
    "Северо-Кавказский": { "Республика Дагестан":                         0,
                           "Республика Ингушетия":                        0,
                           "Кабардино-Балкарская Республика":             0,
                           "Карачаево-Черкесская Республика":             0,
                           "Республика Северная Осетия — Алания":         0,
                           "Чеченская Республика":                        0,
                           "Ставропольский край":                         0 },
    "Приволжский":       { "Республика Башкортостан":                     0,
                           "Республика Марий Эл":                         0,
                           "Республика Мордовия":                         0,
                           "Республика Татарстан":                        0,
                           "Удмуртская Республика":                       0,
                           "Чувашская Республика":                        0,
                           "Пермский край":                               0,
                           "Кировская область":                           0,
                           "Нижегородская область":                       0,
                           "Оренбургская область":                        0,
                           "Пензенская область":                          0,
                           "Самарская область":                           0,
                           "Саратовская область":                         0,
                           "Ульяновская область":                         0 },
    "Уральский":         { "Курганская область":                          0,
                           "Свердловская область":                        0,
                           "Тюменская область":                           0,
                           "Челябинская область":                         0,
                           "Ханты-Мансийский автономный округ — Югра":    0,
                           "Ямало-Ненецкий автономный округ":             0 },
    "Сибирский":         { "Республика Алтай":                            0,
                           "Республика Тыва":                             0,
                           "Республика Хакасия":                          0,
                           "Алтайский край":                              0,
                           "Красноярский край":                           0,
                           "Иркутская область":                           0,
                           "Кемеровская область — Кузбасс":               0,
                           "Новосибирская область":                       0,
                           "Омская область":                              0,
                           "Томская область":                             0 },
    "Дальневосточный":   { "Республика Бурятия":                          0,
                           "Республика Саха (Якутия)":                    0,
                           "Забайкальский край":                          0,
                           "Камчатский край":                             0,
                           "Приморский край":                             0,
                           "Хабаровский край":                            0,
                           "Амурская область":                            0,
                           "Магаданская область":                         0,
                           "Сахалинская область":                         0,
                           "Еврейская автономная область":                0,
                           "Чукотский автономный округ":                  0 }
}
b = {**a["Дальневосточный"], **a["Приволжский"], **a["Северо-Западный"], **a["Северо-Кавказский"], **a["Сибирский"], **a["Уральский"], **a["Центральный"], **a["Южный"]}
for i in a.keys():
    for j in a[i].keys():
        a[i][j] = [0, b, 0, b]
file = open("data2.json", "w", encoding='utf-8')
print(a)
file.write(str(a).replace('\'', '"').replace('Город федерального значения ', ''))
file.close()